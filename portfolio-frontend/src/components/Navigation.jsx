...
const Navigation = () => {
  ...
  const navLinks = [
    ...
    {
      href: "/resume.pdf/Adarsh-kumar-Resume.pdf",
      label: "Get My Resume",
      color: "text-[#5cbcbc]",
      isResume: true,
    },
  ...
  return (
    <nav
      className={`flex items-center justify-between px-4 sm:px-8 lg:px-22 py-4 shadow-sm fixed w-full top-0 z-50 font-sans bg-[#e0e8f1] transition-all duration-500 ${
        isScrolled
          ? "bg-gradient-to-r from-blue-50 to-indigo-50 backdrop-blur-lg border-b border-white/20 shadow-xl py-3"
          : "bg-[#e0e9f1]"
    ...
    >
      ...
      <div className="hidden md:flex items-center space-x-6 lg:space-x-8 font-medium">
        {navLinks.map((link, index) => (
          <a
            ...
            onClick={link.isResume ? handleResumeClick : undefined}
          >
            {link.label}
      ...
      {/* Mobile Menu */}
      <div
        className={`absolute top-full left-0 w-full bg-[#f5f7fa] shadow-lg md:hidden transition-all duration-500 ${
          isMenuOpen ? "opacity-100 translate-y-0 max-h-96" : "opacity-0 -translate-y-10 max-h-0 overflow-hidden"
        }`}
      >
        <div className="flex flex-col py-4 px-6 space-y-4">
          {navLinks.map((link, index) => (
            <a
              ...
              onClick={(e) => {
                if (link.isResume) handleResumeClick(e);
                setIsMenuOpen(false);
              }}
            >
              {link.label}
